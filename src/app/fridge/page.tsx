                  ...
                  <div className="fridge-door-shelves">
                    {door.shelves.map(shelf => (
                      <div key={shelf.id} className="fridge-shelf">
                        ...
                        <div className={`fridge-shelf-slots${viewMode === 'visual' ? ' fridge-shelf-slots--visual' : ''}`} style={{ gridTemplateColumns: `repeat(${shelf.slots}, 1fr)` }}>
                          {Array.from({ length: shelf.slots }).map((_, si) => {
                            ...
                            return (
                              <button
                                ...
                                disabled={!item}
                                onPointerDown={e =>
                                  handleSlotPointerDown(e, door.id, door.name, shelf.id, shelf.name, si)
                                }
                                onPointerUp={e =>
                                  handleSlotPointerUp(e, door.id, door.name, shelf.id, shelf.name, si)
                                }
